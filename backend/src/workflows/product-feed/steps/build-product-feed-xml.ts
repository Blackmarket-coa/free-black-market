  ...
  async (input: StepInput) => {
    ...
    const itemsXml = input.items.map((item) => {
      return (
        ...
        `<g:brand>${escape(item.brand || siteName)}</g:brand>` +
    ...
    const xml =
      ...
      `<channel>` +
      `<title>${escape(siteName)} Product Feed</title>` +
      `<description>Product Feed for Social Platforms and Search Engines</description>` +
      itemsXml +
      `</channel>` +
      `</rss>`
