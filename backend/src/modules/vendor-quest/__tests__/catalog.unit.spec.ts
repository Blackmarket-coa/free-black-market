/**
 * Whole-catalog test: every quest definition (Q1–Q16) runs through the SAME
...
describe("full quest catalog", () => {
  it("has all 16 quests across the four categories", () => {
    ...
    // keys are unique
    expect(new Set(defs.map((d) => d.key)).size).toBe(16)
  })
