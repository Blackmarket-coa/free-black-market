...
const log = createLogger(
  "api/v1/integrations/blackout/commerce/checkout/sessions",
);
...
const BodySchema = z
  .object({
    ...
    // Charge this instead of the listing's own price. A coalition drive's
    // listing is a destination rather than a price — the contributor picks the
    // amount — so without this the card was charged the listing price while
    // Blackout recorded and displayed the amount the contributor actually chose.
    // Capped at $1,000,000 so a malformed caller cannot open a checkout for an
    // amount no one intends to pay.
    amountCents: z.number().int().min(50).max(100_000_000).optional(),
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (!secret) {
    return res
      .status(500)
      .json({ code: "server_error", message: "JWT_SECRET is not configured" });
  }
  ...
  if (!listing) {
    return res
      .status(404)
      .json({ code: "listing_not_found", message: "Listing not found" });
  }
  if (
