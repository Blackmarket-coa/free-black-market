...
const BodySchema = z
  .object({
    ...
    // Stamps the listing carries for consumers that need to know what it is.
    // A coalition drive must arrive with coalition_id and drive_id: the embed
    // drive checkout refuses a listing without them, so one created without
    // them is purchasable through §5 and rejected by the embed. Bounded because
    // this is a write from another service into an opaque column.
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (!sellerId) {
    return res
      ...
      .json({
        code: "not_found",
        message: "No seller matched the supplied identifier",
      });
  }
  ...
  if (existing.length > 0) {
    return res
      ...
      .json({
        code: "duplicate_slug",
        message: "A listing with that slug already exists",
      });
  }
  ...
  return res
    .status(201)
    .json({ id: listing.id, slug: listing.slug, status: listing.status });
}
