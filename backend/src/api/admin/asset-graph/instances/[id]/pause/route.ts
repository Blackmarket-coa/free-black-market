...
/**
 * POST /admin/asset-graph/instances/:id/pause
 *
 * Transition active → paused. 409 on any other source state.
 */
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    const instance = await (service as AssetGraphService).pauseInstance({
      instance_id: id,
    })
