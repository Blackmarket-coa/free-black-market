...
/**
 * POST /admin/asset-graph/instances/:id/archive
 *
 * Terminal transition. draft / active / paused → archived.
 * 409 if the instance is already archived.
 */
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    const instance = await (service as AssetGraphService).archiveInstance({
      instance_id: id,
    })
