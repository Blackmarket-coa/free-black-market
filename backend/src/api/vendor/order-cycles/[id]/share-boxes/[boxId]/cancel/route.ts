...
/**
 * POST /vendor/order-cycles/:id/share-boxes/:boxId/cancel
 *
 * `cancelShareBox` writes `status` unconditionally, so the transition is checked
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  if (!guardTransition(res, OrderCycleModuleService, owned.box.status, "cancelled")) {
    return
  }

  const service = req.scope.resolve<OrderCycleModuleService>(ORDER_CYCLE_MODULE)
  const updated = await service.cancelShareBox(owned.box.id)
