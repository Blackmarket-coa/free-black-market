...
|Order Cycles share-box scheduler (extends `order-cycle` + `food-distribution`)|60–75%  |Corrected 2026-09-08, revised 2026-09-09. The cycle half is reachable (vendor panel, status cron, `cycle.open`/`cycle.close` to Blackout). The share-box half now has routes end to end — templates, subscriptions, generation, pack, dispatch, cancel — and members can browse and order from a cycle, with the sale recorded against it. **What is still missing is the money path**: `share_box_subscription` has no price, payment method, cart, order or ledger link, and share-box pricing falls back to 0 with a `usd` currency. See `docs/CSA_NETWORK_POSITIONING.md` §3|
