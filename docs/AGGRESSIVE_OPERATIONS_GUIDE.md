...
|Order Cycles share-box scheduler (extends `order-cycle` + `food-distribution`)|25–50%  |Corrected 2026-09-08: the cycle half is reachable (vendor panel, status cron); the share-box half is service methods only — templates, subscriptions, box generation, pack and dispatch have no route, job, workflow or screen, and no money path. See `docs/CSA_NETWORK_POSITIONING.md` §3|
