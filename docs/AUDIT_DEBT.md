...
| LR-3 | Eliminate type drift in admin-panel typecheck (separate from `pnpm test` translation validation, which now passes). **Partially landed 2026-05-13** (6 sub-passes): passes 1-5 covered the missing devDeps + missing files + `requests/` + `tax-regions/` + `RequestStatus` dedup + `components/table/`/`hooks/table/`/`locations/`/`regions/`/`reservations/`/`price-lists/` + `hooks/api/`/`inventory/`/`customers/`/phase0 (710 → 472); pass 6 started on the largest residual subtree (`orders/`, 308 errors) and cleared `order-timeline.tsx` (38 → 0) and `order-receive-return-form/` cluster (35 → 0) (472 → 399) — un-nested an `(BaseClaimItem[] \| BaseExchangeItem[])[]` array bug in the `Activity.itemsToSend` type that masked 6 downstream errors, widened `Activity.title` and `OrderActivityItem.title` to `ReactNode` (the update-order branches render `<ChangeDetailsTooltip />` inline), accepted both `AdminFulfillment \| AdminOrderFulfillment` on `FulfillmentCreatedBody` with a structural cast for the `items` access the SDK type omits on the order-embedded variant, cast around SDK omissions for `AdminOrderItem.quantity`, `AdminOrderChange.created_by`, `AdminOrder.canceled_at`, `update.actions[0].details.*` (typed `unknown`), and `AdminOrderLineItem.actions` (the `+actions.action,+actions.details.*` field-expansion the response actually returns) via a `LineItemWithActions` local alias, escaped the i18next typed-key union with `as any` for `orders.activity.events.edit.${edit.status}` template-literal keys, dropped a misjudged `_NOTE_LIMIT` `_` prefix (TS6133 doesn't honor `_` for `noUnusedLocals`) and typed the placeholder `notes` array, null-guarded `update.actions[0]?.details?.*` chains, narrowed `e: unknown` in 4 more catches, supplied `null` fallback to `useStockLocation(orderReturn.location_id ?? "")`, fixed `value` props on quantity inputs to coerce `null \| undefined` → `""`/`null`. The 399 residual errors live in Medusa-inherited routes (`orders/**` ~235 still, `promotions/**` ~53, `products/**` ~40, `product-variants/**` ~32, plus small clusters); they do not block runtime (`vite build` ignores TS errors) but do block flipping CI to fail-fast. CI step remains `continue-on-error: true`. | `admin-panel/src/routes/{orders,promotions,products,product-variants}/**` | 399 errors across ~100 files | admin-panel team | M | `v1.0.0` |
