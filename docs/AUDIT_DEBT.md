...
| LR-3 | Eliminate type drift in admin-panel typecheck (separate from `pnpm test` translation validation, which now passes). **Partially landed 2026-05-13** (4 sub-passes): pass 1 installed missing devDeps + restored 3 missing local files + repointed dead imports (710 → 671); pass 2 cleared `requests/` (671 → 629); pass 3 cleared `tax-regions/` + deduplicated `RequestStatus` (629 → 604); pass 4 cleared `components/table/`, `hooks/table/`, `locations/`, `regions/`, `reservations/`, `price-lists/` (604 → 535) — installed `@types/lodash`, exported `ViewConfiguration` from `use-view-configurations`, swapped `SquarePlusMicro` → `SquaresPlus` (icon rename), fixed `view-selector` to read response fields flat (no `.data` envelope), narrowed `ViewConfiguration` via `NonNullable`, widened `getFormattedAddress` to a structural shape so `AdminStockLocationAddress` + `AdminOrderAddress` both work, fixed SDK type renames (`AdminOrderColumn` → `AdminColumn`, `TaxRateResponse` → `AdminTaxRate`, `AdminFulfillmentProviderListParams` → `AdminGetFulfillmentProvidersParams`), cast over `AdminOrderFilters` / `AdminProductListParams` SDK omissions, narrowed `PriceRuleOperator` enum + dropped string conversion in `price-rule-helpers`, widened `rules[]` to `AdminUpdate | AdminCreate` union, suppressed dead order/payment-status filters via `@ts-expect-error`, repointed regions screens from `AdminRegionCountry`/`RegionCountryDTO` → `StaticCountry` (the actual shape `useCountries`/`useCountryTableColumns` work with), coerced 6 `T[] | undefined` → `T[]` on price-list form props, dropped invalid `customer_group_ids` from a zod schema's `.pick()`, plus several null-guards. The 535 residual errors are real type drift in Medusa-inherited routes (`orders/**` ~312, `promotions/**` ~54, `products/**` ~40, `product-variants/**` ~32, `hooks/api/**` ~29, `inventory/**` ~17, plus small clusters); they do not block runtime (`vite build` ignores TS errors) but do block flipping CI to fail-fast. CI step remains `continue-on-error: true`. | `admin-panel/src/routes/{orders,promotions,products,product-variants,inventory}/**`, `admin-panel/src/hooks/api/**` | 535 errors across ~150 files | admin-panel team | M | `v1.0.0` |
