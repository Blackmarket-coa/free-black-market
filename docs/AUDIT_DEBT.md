...
|---|------|----------|------:|-------|:------:|------------------|
| TS-1 | Replace `any` in API hooks, table/grid utilities, dashboard extensibility | `admin-panel/src/hooks/api/*.tsx`, `admin-panel/src/components/data-grid/*`, `admin-panel/src/lib/table/*`, `admin-panel/src/types/*`, `admin-panel/src/dashboard-app/*` | 5282 lint warnings (down from 662 explicit-any after Medusa SDK retype + route-boundary narrowing; `src/hooks/api/*.tsx` now clean; remaining concentrate in data-grid generics & route-form schemas) | admin-panel team | L | post-`v1.0.0` |
...
| ~~LR-3~~ | ~~Eliminate translation-contract drift in admin-panel typecheck — missing modules `@medusajs/admin-sdk`, `@medusajs/framework`, `stripe` installed at `2.12.5`/latest; local types defined for `TicketProduct`, `Venue`, `Donation*` in `admin-panel/src/types/{ticket-product,venue,donation}`; `@medusajs/framework/types::OrderCreditLineDTO` import resolves; downstream route-level mismatches (orders, tax-regions, products, regions, reservations, promotions, requests, inventory) cleared via narrowed casts at SDK boundaries. `admin-panel/package.json` `lint` ratcheted from `--max-warnings 7000` → `5300`. `Typecheck admin panel` CI step is now blocking (`continue-on-error: true` removed).~~ | `admin-panel/src/types/**`, `admin-panel/src/routes/**` | 0 errors | admin-panel team | — | done |
