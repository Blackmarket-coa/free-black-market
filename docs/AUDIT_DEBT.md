...
| LR-3 | Eliminate type drift in admin-panel typecheck (separate from `pnpm test` translation validation, which now passes). **Partially landed 2026-05-13** (9 sub-passes, **77% cleared**): passes 1-5 covered the missing devDeps + missing files + 12 cluster subtrees (710 → 472); passes 6-8 cleared most of `orders/**` (472 → 263); **pass 9 fully cleared the `orders/` subtree** (263 → 164): return-create-form (16), claim-outbound-section (10), exchange-outbound-section (9), order-create-fulfillment-item (12), order-active-edit-section (8), order-receive-return.tsx (5), return-create.tsx (4), order-payment-section (4), order-note-form (4), exchange-inbound-section (3), order-detail.tsx (3), add-exchange-inbound-items-table (3), order-create-shipment-form (2), order-table-adapter (2), claim/exchange/edit-create catch-error tails, and the 3 `*-inbound-item.tsx` files. Highlights this pass: cast around SDK omissions for `AdminOrderLineItem.return_requested_total`, `AdminOrderChangeAction.amount`, `AdminFulfillmentItem.line_item_id` (string \| null), `AdminInventoryItem.variant_id`/`product_id` (string \| null on AdminOrderLineItem), `AdminOrder.no_notification` / `canceled_at`, `AdminRefund.payment_id`, the `return_receive` AdminOrderChangeType variant; widened `outbound_items` FieldArray rows to carry `variant_id` via structural cast (the schema doesn't declare it); cast `AdminReturnResponse` envelope in `initiateReturn` callers; switched React-Query v5 `placeholderData` to read `previousQuery.queryKey[…]` instead of indexing the Query object; widened `OrderActiveEditSection.quantity` to optional; non-null-asserted document.getElementById; coerced multiple `.find(Boolean)` action menus through `any[]`; narrowed many more `e: unknown` catches; replaced a placeholder `{}` mutateAsync stub with a typed signature. The 164 residual errors live in Medusa-inherited routes (`promotions/**` ~53, `products/**` ~40, `product-variants/**` ~32, plus small clusters); they do not block runtime (`vite build` ignores TS errors) but do block flipping CI to fail-fast. CI step remains `continue-on-error: true`. | `admin-panel/src/routes/{promotions,products,product-variants}/**` | 164 errors across ~40 files | admin-panel team | M | `v1.0.0` |
