...
| LR-3 | Eliminate type drift in admin-panel typecheck (separate from `pnpm test` translation validation, which now passes). **Partially landed 2026-05-13** (3 sub-passes): pass 1 installed missing devDeps + restored 3 missing local files + repointed dead imports (710 → 671); pass 2 cleared `requests/` subtree (renamed `request.payload` → `request.data` across 11 files, added missing legacy fields to `SellerRequestData`, narrowed `AdminSellerRequest` typing in the seller-list state) (671 → 629); pass 3 cleared `tax-regions/` subtree (suppressed 3 dead-component TS6133 via `@ts-expect-error`, fixed `DataTableOrderBy` callers to pass `{key,label}` shapes, coerced null/undefined mismatches in form-default-values, fixed `useShippingOptionTableQuery` props optionality, wrapped React-Query `mutateAsync` for `MetaDataSubmitHook`, cast `AdminTaxRateRule.created_at` to access undeclared SDK field, optionalised `initialValues` in `useDefaultRulesValues` return type, narrowed the queryResults result-union with an `any` cast, added `value` prop to TargetItem call) (629 → 605); plus deduplicated divergent `RequestStatus` declaration (605 → 604). The 604 residual errors are real type drift in Medusa-inherited routes (`orders/**` ~312, `promotions/**` ~54, `products/**` ~40, `product-variants/**` ~32, `hooks/api/**` ~29, `inventory/**` ~17, `locations/**` ~15, `components/table/**` ~14, plus small clusters); they do not block runtime (`vite build` ignores TS errors) but do block flipping CI to fail-fast. CI step remains `continue-on-error: true`. | `admin-panel/src/routes/{orders,promotions,products,product-variants}/**`, `admin-panel/src/hooks/api/**`, `admin-panel/src/components/{table,data-grid,filtering}/**` | 604 errors across ~180 files | admin-panel team | M | `v1.0.0` |
