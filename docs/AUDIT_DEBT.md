...
| LR-3 | Eliminate type drift in admin-panel typecheck (separate from `pnpm test` translation validation, which now passes). **Partially landed 2026-05-13** (8 sub-passes): passes 1-5 covered the missing devDeps + missing files + 12 cluster subtrees (710 → 472); pass 6 cleared `order-timeline.tsx` + `order-receive-return-form/` (472 → 399); pass 7 cleared `order-summary-section/` + `return-info-popover` + `order-fulfillment-section` + `use-order-data-table-columns` + `exchange-create-form` + claim-create-form + order-create-fulfillment-form (399 → 342); pass 8 cleared `order-edit-item.tsx` + `order-edit-items-section.tsx` + `add-claim-items-table.tsx` + `add-return-items-table.tsx` + the allocate-items pair (`order-allocate-items-form.tsx` + `order-allocate-items-item.tsx`) (342 → 263, 63% cleared cumulative). Highlights this pass: replicated the `LineItemWithActions` local alias for the actions field-expansion in order-edit-item; cast around AdminOrderLineItem `returned_quantity` / `refundable` SDK omissions in the two add-*-items-table files and dropped the unused `getStylizedAmount` import / `currency_code` parameter; coerced `Number()` on numeric operator comparisons (`gt`/`gte`/`lt`/`lte`); null-coerced `i.product_title?.toLowerCase()` chains; extended the `checkInventoryKit` helper in `utils.ts` to accept either `OrderLineItemDTO` or `AdminOrderLineItem` with the `variant.inventory_items` join cast structurally; captured `LineItemWithVariant` / `ItemWithInventory` local aliases in the allocate-items pair for the response-only `variant.inventory[].location_levels` join; bypassed the `useForm.setValue` typed name union with localized `as any` for the dynamic `quantity.${string}-${string}` keys; cast around `InventoryItemDTO.location_levels` SDK omission. The 263 residual errors live in Medusa-inherited routes (`orders/**` ~99, `promotions/**` ~53, `products/**` ~40, `product-variants/**` ~32, plus small clusters); they do not block runtime (`vite build` ignores TS errors) but do block flipping CI to fail-fast. CI step remains `continue-on-error: true`. | `admin-panel/src/routes/{orders,promotions,products,product-variants}/**` | 263 errors across ~65 files | admin-panel team | M | `v1.0.0` |
