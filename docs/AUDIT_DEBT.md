...
| LR-3 | Eliminate type drift in admin-panel typecheck (separate from `pnpm test` translation validation, which now passes). **Partially landed 2026-05-13** (7 sub-passes): passes 1-5 covered the missing devDeps + missing files + 12 cluster subtrees (710 → 472); pass 6 cleared `order-timeline.tsx` + `order-receive-return-form/` (472 → 399); pass 7 cleared `order-summary-section/` + `return-info-popover` + `order-fulfillment-section` + `use-order-data-table-columns` + `exchange-create-form` + most of `claim-create-form` + most of `order-create-fulfillment-form` (399 → 342). Highlights: cast around SDK omissions for `AdminReturnItem.reason`, `BaseClaimItem.item` / `BaseExchangeItem.item`, `AdminReturn.requested_at`, `AdminProductVariant.inventory`, `AdminOrderChangeAction.amount`, `AdminOrder.no_notification`; widened `Fulfillment` prop to `AdminOrderFulfillment & Partial<AdminFulfillment>` with a cast at the call site; null-coerced `item.variant?.options?` chains; converted `getLocaleAmount(item.subtotal)` to `Number(item.subtotal ?? 0)` for the `BigNumberValue` → number argument; removed unsupported `enableHiding` / `enableSorting` from the new DataTable column-defs (the `@medusajs/ui` v4 `DataTableIdentifiedColumnDef` only allows `meta` / `header` / `cell` / `min/maxSize` / `size` / `id`); replaced the `variant="default"` StatusBadge prop with the v4 `color="grey"` API; widened `getFulfillableQuantity(item)` to accept either `OrderLineItemDTO` or `AdminOrderLineItem` in `src/lib/order-item.ts`; made `OrderCreateFulfillmentItem`'s `currencyCode` / `onItemRemove` props optional (they're not consumed at the only call site); narrowed `e: unknown` and coerced `T[] | undefined` in several more catch / prop-pass spots; dropped the `dismissLabel` toast prop and the `classNameInner` Alert prop that v4.0.28 no longer accepts. The 342 residual errors live in Medusa-inherited routes (`orders/**` ~178, `promotions/**` ~53, `products/**` ~40, `product-variants/**` ~32, plus small clusters); they do not block runtime (`vite build` ignores TS errors) but do block flipping CI to fail-fast. CI step remains `continue-on-error: true`. | `admin-panel/src/routes/{orders,promotions,products,product-variants}/**` | 342 errors across ~80 files | admin-panel team | M | `v1.0.0` |
