...
| TD-3 | Sell signup → API integration. **Partially landed 2026-05-13**: client now best-effort POSTs `{email, store_name, selling[]}` to `/api/sell-signup` before redirecting to the vendor-panel registration page, and the Next.js route stub at `storefront/src/app/api/sell-signup/route.ts` validates + logs the payload server-side and returns 202 Accepted. Remaining work: wire the route to a backend leads endpoint or webhook so captures are durable rather than log-only. | `storefront/src/app/[locale]/(main)/sell/SellPageClient.tsx`, `storefront/src/app/api/sell-signup/route.ts` | 1 | storefront team | S | `v1.0.0` |
...
|---|------|----------|--------:|-------|:------:|------------------|
| LR-1 | Lower admin-panel ESLint `--max-warnings` from 7000 → 0 in steps (~~5500~~ → 4000 → 2000 → 0). **Step 1 done 2026-05-13**: ratcheted 7000 → 5500 after running `pnpm exec eslint --fix` to clear 1,586 auto-fixable warnings (mostly `@typescript-eslint/consistent-type-imports` and `newline-before-return`); count is now 4,106 with `lint` exit 0. Next step is to clear ~107 more warnings (likely from `@typescript-eslint/no-unused-vars` / `react-hooks/exhaustive-deps`) and ratchet to 4000. | `admin-panel/package.json` `lint` script | 5500 cap (currently 4,106 warnings) | admin-panel team | L | `v1.0.0` → `v1.2.0` |
...
| LR-3 | Eliminate translation-contract drift in admin-panel typecheck (separate from `pnpm test` translation validation, which now passes). **Partially landed 2026-05-13**: installed missing devDeps (`@medusajs/admin-sdk@2.12.5`, `@medusajs/framework@2.12.5`, `@sentry/browser`, `stripe`), restored 3 missing local files (`src/types/venue.ts`, `src/types/ticket-product.ts`, `src/components/create-venue-modal.tsx` as a single-row stub), repointed `src/index.ts` from the nonexistent `./render` to `./dashboard-app`, and fixed the `tax-region-province-detail/components/index.ts` paths. Errors dropped from **710 → 671**. The 671 residual errors are real type drift in Medusa-inherited routes (`orders/**`, `product-variants/**`, `promotions/**`, `tax-regions/**`) and unused-import / strict-null cleanups; they do not block runtime (`vite build` ignores TS errors) but do block flipping CI to fail-fast. CI step remains `continue-on-error: true`. | `admin-panel/src/routes/{orders,product-variants,promotions,tax-regions}/**`, `admin-panel/src/components/{table,data-grid,filtering}/**` | 671 cascaded errors across 199 files | admin-panel team | M | `v1.0.0` |
