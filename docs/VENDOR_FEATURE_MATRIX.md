...
| Messaging/support chat | Matrix, not Rocket.Chat: `backend/src/api/vendor/chat`, `backend/src/api/store/chat` (Element auto-login tokens via `backend/src/shared/matrix-service.ts`) | `vendor-panel/src/routes/messages` via `vendor-panel/src/providers/matrix-provider` | `storefront/src/components/sections/UserMessagesSection` via `storefront/src/providers/MatrixChatProvider.tsx` | 🟡 |
