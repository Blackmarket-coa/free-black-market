...
| Invoicing | `backend/src/api/vendor/invoices` supports list/create/update lifecycle (`draft/sent/paid/void`) and legacy queue contract mode | Invoicing flow wired in `vendor-panel/src/routes/finances/finances.tsx` (`/finances?view=invoicing`) | Invoices can be associated with order IDs and feed POS payment references | ✅ |
| POS mode | POS API support added at `backend/src/api/vendor/pos/config` and `backend/src/api/vendor/pos/checkout` with endpoint map to existing payment/order rails | Dedicated POS screen at `vendor-panel/src/routes/pos/*` and route wiring in router map | POS checkout interoperates with existing hawala vendor payment pipeline | ✅ |
