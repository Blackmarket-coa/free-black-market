...
(function (window, document) {
  ...
  var config = {
    ...
    // Coalition drives are vendorless and read from Blackout, so they carry
    // their own base URL and default coalition alongside the vendor config.
    blackout: stripSlash(attr("blackout", "https://theblackout.app")),
    coalition: attr("coalition", ""),
  };
  ...
  // ---------------------------------------------------------------------------
  // Coalition drives — vendorless, like the buyer hub
  //
  // A drive belongs to a coalition on Blackout, not to an FBM vendor, so there
  // is no handle to hang it off and no vendor capability to gate it on: a
  // member embeds their coalition's drive on their own site without
  // configuring a vendor at all.
  //
  // The read is a public, cacheable Blackout endpoint. The donate leg goes
  // through the SAME hosted checkout every other embedded purchase uses
  // (POST /v1/integrations/blackout/commerce/checkout/sessions -> /page?embed=1),
  // so the money path, the 3% commission and the postMessage protocol are the
  // ones already in production — this widget adds a surface, not a rail.
  // ---------------------------------------------------------------------------
  function blackoutBase() {
    return stripSlash(config.blackout || "https://theblackout.app");
  }

  function getDrive(opts) {
    opts = opts || {};
    var coalition = opts.coalition || config.coalition;
    var drive = opts.drive;
    if (!coalition || !drive) {
      return Promise.reject(
        new Error("FBM: a drive widget needs data-fbm-coalition and data-fbm-drive")
      );
    }
    var url =
      blackoutBase() +
      "/v1/coalitions/" +
      encodeURIComponent(coalition) +
      "/campaigns/" +
      encodeURIComponent(drive) +
      "/public";
    return fetch(url, { credentials: "omit" })
      .then(function (res) {
        if (!res.ok) throw new Error("FBM: drive request failed (" + res.status + ")");
        return res.json();
      })
      .then(function (data) {
        return (data && data.drive) || null;
      });
  }

  function driveAmounts(opts) {
    var raw = (opts && opts.amounts) || "10,25,50";
    var out = [];
    String(raw)
      .split(",")
      .forEach(function (part) {
        var n = parseFloat(String(part).trim());
        // Reject anything that is not a positive number rather than rendering
        // a NaN button that would mint a broken checkout session.
        if (isFinite(n) && n > 0) out.push(Math.round(n * 100));
      });
    return out.length ? out : [1000, 2500, 5000];
  }

  /**
   * Open a donation checkout for `amountCents`. Returns the session URL, or
   * null when the drive has no listing behind it yet — in which case the
   * caller sends the visitor to the coalition page rather than pretending a
   * checkout exists.
   */
  function donate(drive, amountCents) {
    if (!drive || !drive.fbmListingId) return Promise.resolve(null);
    var url = config.api + "/store/embed/drives/checkout";
    return fetch(url, {
      method: "POST",
      headers: (function () {
        var h = authHeaders();
      ...
      credentials: "omit",
      body: JSON.stringify({
        coalition_id: drive.coalitionId,
        drive_id: drive.id,
        listing_id: drive.fbmListingId,
        amount_cents: amountCents,
      }),
    })
      .then(function (res) {
        if (!res.ok) throw new Error("FBM: donate request failed (" + res.status + ")");
        return res.json();
      })
      .then(function (data) {
  ...
  function driveCard(d) {
    var goal = Number(d.goalCents || 0);
    var raised = Number(d.raisedCents || 0);
    // Guard the divide: a drive with no goal would render NaN%.
    var pct = goal > 0 ? Math.min(100, Math.round((raised / goal) * 100)) : 0;
    var amounts = driveAmounts(d._opts);

    var buttons = amounts
      .map(function (cents) {
        return (
          '<button type="button" class="fbm-cta fbm-drive__amount" data-fbm-amount="' +
          cents +
          '">' +
    ...
    return (
      '<div class="fbm-card fbm-drive">' +
      '<div class="fbm-card__body">' +
      '<p class="fbm-card__title">' + escapeHtml(d.title || "Drive") + "</p>" +
      '<p class="fbm-card__meta">' + escapeHtml(d.coalitionName || "") + "</p>" +
      (goal > 0
        ? '<div class="fbm-pool__bar"><span style="width:' + pct + '%"></span></div>' +
          '<p class="fbm-card__meta">' +
          escapeHtml(
            formatPrice(raised / 100) +
              " of " +
              formatPrice(goal / 100) +
              " · " +
          ...
          "</p>"
        : '<p class="fbm-card__meta">' +
          escapeHtml(formatPrice(raised / 100) + " raised") +
          "</p>") +
      (d.status === "completed"
        ? '<p class="fbm-card__meta">This drive has closed.</p>'
        : '<div class="fbm-drive__amounts">' + buttons + "</div>") +
      '<a class="fbm-card__meta" href="' + escapeHtml(d.url || blackoutBase()) + '" target="_blank" rel="noopener">Open this drive on Blackout</a>' +
      "</div></div>"
    );
  }

  function renderDrive(target, opts) {
    var node = resolveEl(target);
    if (!node) return Promise.resolve();
    opts = opts || {};
    return renderInto(
      node,
      getDrive(opts).then(function (d) {
        if (!d) return [];
        d._opts = opts;
        return [d];
      }),
      driveCard,
      "This drive is not open right now."
    ).then(function () {
      wireDriveButtons(node, opts);
    });
  }

  function wireDriveButtons(node, opts) {
    var buttons = node.querySelectorAll("[data-fbm-amount]");
    if (!buttons.length) return;
    Array.prototype.forEach.call(buttons, function (button) {
      button.addEventListener("click", function () {
        var cents = parseInt(button.getAttribute("data-fbm-amount") || "", 10);
        if (!isFinite(cents) || cents <= 0) return;
        track("donate_start", { drive_id: opts.drive, amount_cents: cents });
        getDrive(opts)
          .then(function (d) {
            return donate(d, cents).then(function (url) {
              // No listing behind the drive yet: send the visitor to the
              // coalition page instead of opening a checkout that cannot work.
              openModal(url || (d && d.url) || blackoutBase());
            });
          })
          .catch(function () {
            openModal(blackoutBase());
          });
      });
    });
  }
  ...
  /**
   * Checkout outcomes arrive from the hosted page as postMessages. We only
   * trust messages that name themselves as FBM checkout events; anything else
   * on the channel is ignored.
   */
  window.addEventListener("message", function (event) {
    var data = event && event.data;
    if (!data || data.source !== "fbm-checkout") return;
    if (data.type === "checkout.completed") {
      track("donate_complete", {});
      emit("donation:complete", data.payload || {});
    } else if (data.type === "checkout.cancelled") {
      emit("donation:cancelled", data.payload || {});
    }
  });
  ...
  function injectStyles() {
    ...
    var css =
      ...
      ".fbm-drive__amounts{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0}" +
      ".fbm-drive__amount{flex:1 1 auto;min-width:72px}" +
  ...
  function renderKind(kind, node, opts) {
    ...
    else if (kind === "drive") renderDrive(node, opts);
  }
   ...
   */
  var VENDORLESS_KINDS = { "demand-pools": true, drive: true };
  ...
  function autoMount(root) {
    ...
    Array.prototype.forEach.call(nodes, function (node) {
      ...
      var opts = {
        ...
        coalition: node.getAttribute("data-fbm-coalition") || config.coalition || undefined,
        drive: node.getAttribute("data-fbm-drive") || undefined,
        amounts: node.getAttribute("data-fbm-amounts") || undefined,
      };
