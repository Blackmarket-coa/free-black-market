...
- [x] **SEC-2/3** ~~Authenticate the community/food write routes and market creation.~~ — **done**: write verbs on ~16 `/store` community/food prefixes now require a logged-in account (customer/seller/driver) via matchers in `api/middlewares.ts`. **Per-handler owner-id scoping also done**: 10 handlers that trusted a body `customer_id`/`*_by_id` now bind it to the authenticated actor (`shared/actor-scope.ts`), and the vote/claim/signup/volunteer-log handlers reject a borrowed `membership_id`. **CLASS-B entity-ownership (partial):** `POST /store/food-producers` and `POST /store/couriers` now reject attaching a `hawala_account_id` the actor doesn't own (`hawalaAccountOwnershipError` — the ledger account is `owner_type`/`owner_id`-linked). The rest is schema-blocked and tracked separately: `food_courier` has **no owner column** (so `/store/couriers/[id]/**` writes and `delivery-batches` courier refs can't be ownership-checked without a migration), and customer-created producers don't stamp an owner. *(Backend)*
