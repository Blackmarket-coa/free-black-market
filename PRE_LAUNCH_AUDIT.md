...
|---|---|---|---|
| LEG-1 | **Critical** | ~~**No Terms of Service, Privacy Policy, or Refund/Returns policy pages exist** anywhere in the storefront, and none are linked at registration/checkout.~~ **Drafted 2026-09-13 — still gated on operator input and legal review.** `/legal/terms`, `/legal/privacy` and `/legal/refunds` exist, share one shell and one constants module so they cannot state different effective dates or contacts, and are linked from a new footer Legal column, beneath the Create-account button and beneath Place-order. Written against what the code does rather than from a template: the Terms say the contract of sale is with the seller and FBM facilitates the payment, because `docs/POSTURE_A_COMPLIANCE.md` requires an agreement made directly with the seller and boilerplate naming the platform as merchant would contradict the posture the payment architecture rests on; the Refunds page keeps returns (seller's terms) separate from claims (decided by FBM) and carries a perishable-goods carve-out that matters when much of the catalogue is food and live plants; the Privacy page describes the export and delete buttons as they actually behave, which is D11-1. **Not done, and deliberately cannot be marked done:** ten facts are operator decisions and ship as visible `[[TOKENS]]` — legal entity, entity form and state, registered address, governing law, dispute venue, three contact inboxes, and whether the EU/UK is served. `scripts/check-legal-placeholders.mjs` exits non-zero while any token or the not-reviewed banner remains, so this cannot go public half-filled by being forgotten. The drafts have not been seen by a lawyer. | `storefront/src/app/[locale]/(main)/legal/*`; gate: `node scripts/check-legal-placeholders.mjs` |
...
- [ ] **LEG-2 — REOPENED 2026-09-13.** The endpoints and the panel shipped (`GET/POST /store/customers/me/{data-export,deletion}`, "Privacy & your data" in account settings) and that is what this box was ticked for. Writing the Privacy Policy established what they actually reach: the export returns profile, addresses and orders while stating it contains everything held; deletion anonymises the customer record, drops addresses and revokes sign-in and touches nothing else. **32 modules key records on `customer_id`** — a past delivery keeps the recipient's name, phone and street address, and volunteer, garden, ledger, governance and wellness records all survive. Tracked in full as D11-1 in `docs/AUDIT_DEBT.md`; the fix is one registry consumed by both endpoints, not two hand-extended lists.
...
- [x] **INFRA-1** ~~Wire Sentry.~~ — **done, both halves + source maps.** Backend: `@sentry/node` + `initSentry()` in `instrumentation.ts` (activates the existing 5xx capture point). Storefront: `@sentry/nextjs` with static server/edge/`instrumentation-client.ts` configs + `onRequestError`, and `withSentryConfig` in `next.config.ts` for source-map upload (readable stack traces; uploads when `SENTRY_ORG`/`SENTRY_PROJECT`/`SENTRY_AUTH_TOKEN` are set at build). Set `SENTRY_DSN` / `NEXT_PUBLIC_SENTRY_DSN` to turn it on.
...
- [x] **SEC-2/3** ~~Authenticate the community/food write routes and market creation.~~ — **done**: write verbs on ~16 `/store` community/food prefixes now require a logged-in account (customer/seller/driver) via matchers in `api/middlewares.ts`. **Per-handler owner-id scoping also done**: 10 handlers that trusted a body `customer_id`/`*_by_id` now bind it to the authenticated actor (`shared/actor-scope.ts`), and the vote/claim/signup/volunteer-log handlers reject a borrowed `membership_id`. **CLASS-B entity-ownership:** `POST /store/food-producers` and `POST /store/couriers` reject attaching a `hawala_account_id` the actor doesn't own (`hawalaAccountOwnershipError`). **Courier, producer & delivery-batch ownership now enforced end-to-end**: migrations add `owner_id`/`owner_type` to `food_courier`, `food_producer` and `food_delivery_batch`; the creating account is stamped on `POST /store/{couriers,food-producers,delivery-batches}`; and the corresponding `[id]` writes reject a non-owner via `actorMayManage` (grandfathering pre-migration rows) — couriers (update/claim/deactivate + shifts), producers (update/deactivate + producer-side order status transitions), and batches (update/add-deliveries/cancel). Residual (separate, read-path): `GET /store/food-producers/[id]/orders` (list + single) aren't covered by the write-only auth matchers, so a producer's order list — which carries customer PII — is still an unauthenticated read; locking it requires adding those GETs to the auth matchers and is tracked apart from the entity-ownership work. *(Backend)*
