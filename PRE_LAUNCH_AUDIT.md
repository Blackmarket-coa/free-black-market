...
- [x] **SEC-2/3** ~~Authenticate the community/food write routes and market creation.~~ — **done**: write verbs on ~16 `/store` community/food prefixes now require a logged-in account (customer/seller/driver) via matchers in `api/middlewares.ts`. **Per-handler owner-id scoping also done**: 10 handlers that trusted a body `customer_id`/`*_by_id` now bind it to the authenticated actor (`shared/actor-scope.ts`), and the vote/claim/signup/volunteer-log handlers reject a borrowed `membership_id`. **CLASS-B entity-ownership:** `POST /store/food-producers` and `POST /store/couriers` reject attaching a `hawala_account_id` the actor doesn't own (`hawalaAccountOwnershipError`). **Courier ownership now enforced end-to-end**: a migration adds `owner_id`/`owner_type` to `food_courier`, `POST /store/couriers` stamps the creating account, and `/store/couriers/[id]` update/claim/deactivate + shifts reject a non-owner (`actorMayManage`, grandfathering pre-migration rows). Residual (smaller): customer-created *producers* still don't stamp an owner, and `delivery-batches` rows have no owner column — both a follow-up. *(Backend)*
