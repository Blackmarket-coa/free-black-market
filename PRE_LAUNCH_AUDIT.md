...
| INFRA-5 | **Med** | `.env.production.example` is **missing keys the code reads** (`STRIPE_SECRET_API_KEY`/`STRIPE_SECRET_KEY`, `BACKEND_URL`, observability sinks, ~~`CREATOR_ATTRIBUTION_IP_SALT`~~ (added 2026-09-20), several `NEXT_PUBLIC_FF_*`) and has name mismatches (`RESEND_FROM` vs `RESEND_FROM_EMAIL`; `ALGOLIA_ADMIN_API_KEY` vs `ALGOLIA_API_KEY`). Deployment landmines. | grep of `process.env` vs the example |
...
- [x] **LEG-2** ~~Ship account-deletion + data-export (CCPA/CPRA).~~ — **closed 2026-09-13, second time and properly.** First ticked for the endpoints existing; reopened the same day when the Privacy Policy work established they reached 2 of 48 tables while telling the person they reached everything. Now both endpoints consume `backend/src/lib/customer-data-registry.ts`, which names all 47 customer-linked entities with a disposition and a legal basis, and a test fails the build when a model gains a `customer_id` without an entry. Tracked in full as D11-1 in `docs/AUDIT_DEBT.md`.
