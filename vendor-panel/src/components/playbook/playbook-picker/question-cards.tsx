...
type MultiQuestionProps<T extends string> = {
  title: string
  subtitle: string
  options: Option<T>[]
  values: T[]
...
/**
 * Multi-select variant of {@link QuestionCard}. Used by the resource quiz,
 * where each step asks "what do you have?" and the user may pick any number
 * of options (including none). Shares the card styling with the
 * single-select version.
 */
export function MultiQuestionCard<T extends string>({
  title,
  subtitle,
  options,
  values,
  onToggle,
}: MultiQuestionProps<T>) {
  return (
    <div className="flex flex-col gap-y-4 w-full">
      <div className="text-center">
        <h2 className="text-ui-fg-base text-xl font-semibold">{title}</h2>
